    ...
    <>
      ...
      <div className="scroll">
        ...
        <div className="card" style={{ margin: "0 16px", overflow: "hidden" }}>
          ...
          <div className="listrow" data-comment-anchor="7a61510fb3-div-152-11">
...
const readFontBtn = () => {
  const v = parseInt(localStorage.getItem("yh_rd_font"), 10);
  const idx = isNaN(v) ? 1 : v;
  return idx <= 0 ? 0 : idx <= 2 ? 1 : 2;
};
const FontSizePicker = () => {
  const [s, setS] = React.useState(readFontBtn);
  const pick = (i) => {
    setS(i);
    localStorage.setItem("yh_rd_font", String(FONT_BTN_TO_IDX[i]));
  };
  return (
    <div style={{ display: "flex", gap: 6 }}>
      {["小", "中", "大"].map((l, i) =>
      <span key={i} onClick={() => pick(i)} style={{ width: 34, height: 30, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 8,
